...
public class LoginAndSendEmailTest extends BaseTest {
  ...
  //Todo: fix wait for hidden button
  ...
  void createNewMailTest() {
    log.info("Create Email Test invoked");
    EmailPage page = new EmailPage(webDriver);
    page.openPage();

    Assumptions.assumeThat(page.getEmailSpanText())
        .as("Assume check if current page is an email page")
        .isEqualTo("test_user_epam_1234@proton.me");
  }
}
